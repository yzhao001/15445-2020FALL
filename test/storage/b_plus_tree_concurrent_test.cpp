...
void mymix() {
  for (size_t iter = 0; iter < NUM_ITERS; iter++) {
    ...
    InsertHelper(&tree, allvec, 1);
    int64_t size = 0;

    for (auto &pair : tree) {
      int64_t testnum = (pair.first).ToString();
      int64_t insertnum = allvec[size];
      EXPECT_EQ(testnum, insertnum);
      size++;
    }
    // InsertHelper(&tree, for_insert, 1);
    DeleteHelper(&tree, for_delete, 1);
    size = 0;
    for (auto &pair : tree) {
      int64_t testnum = (pair.first).ToString();
      int64_t insertnum = for_insert[size];
      EXPECT_EQ(testnum, insertnum);
      size++;
    }
...
void MixTest1Call() {
  for (size_t iter = 0; iter < NUM_ITERS; iter++) {
    ...
    for (auto &pair : tree) {
      int64_t testnum = (pair.first).ToString();
      int64_t insertnum = for_insert[size];
      if (testnum != insertnum) {
        int a = 0;
        a++;
        assert(0);
        std::cout << " not equal" << std::endl;
      }
      EXPECT_EQ(testnum, for_insert[size]);
      size++;
    }
...
// Score: 5 Description: Concurrently insert a set of keys.
...
// Score: 5 Description: Split the concurrent insert test to multiple threads without overlap.
...
// Score: 5 Description: Concurrently delete a set of keys.
...
// Score: 5 Description: Split the concurrent delete task to multiple threads  without overlap.
 ...
 */
// Score: 5 Description: First insert a set of keys. Then concurrently delete those already inserted keys and
// insert different set of keys. Check if all old keys are deleted and new keys are added correctly.

TEST(BPlusTreeConcurrentTest, mymix) {
  TEST_TIMEOUT_BEGIN
  mymix();
  remove("test.db");
  remove("test.log");
...
// Score: 5
// escription: Insert a set of keys. Concurrently insert and delete a differnt set of keys.
// At the same time, concurrently get the previously inserted keys.
// Check all the keys get are the same set of keys as previously inserted.
...
// Then concurrently delete those already inserted keys and insert different set of keys. Check if all old keys are
